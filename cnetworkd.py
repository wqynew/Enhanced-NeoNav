...
from cmultiprocessing_env import SubprocVecEnv
from scene_loader3 import ActiveVisionDatasetEnv
...
#move (0) or stop (1) 
    def act(self, px1,px2,px3, x, g,pre_action, deterministic=True):
        logit, gc,cc, z_mean, z_sigma, z  = self.forward(px1,px2,px3,x,g,pre_action)
    ...
    def evaluate_actions(self, px1, px2, px3, x, g, action,x_next,pre_action):
        ...
        for i in range(self.batch_size):#num_envs
            if m_or_s[i]==a:
                action[i]=b
        #=========================================================
        return logit,gc,cc, z_mean,z_sigma, z,action 
...
class RolloutStorage(object):
    def __init__(self, num_steps, num_envs, state_shape):
        ...
        self.pre_actions=torch.zeros(num_steps + 1, num_envs, 9)
        self.colli_infos=torch.zeros(num_steps+1,num_envs,7)
    ...
    def cuda(self):
        ...
        self.colli_infos=self.colli_infos.cuda()
    ...
    def insert(self, step, pstate1,pstate2,pstate3,state, g,action, gtaction,gt_state, reward, mask,pre_action,colli_infos):
        ...
        self.colli_infos[step+1].copy_(colli_infos)
    ...
    def after_update(self):
        ...
        self.colli_infos[0].copy_(self.colli_infos[-1])
...
def save_checkpoint(state, global_t):
    ...
    best_path = os.path.join(CHECK_DIR, 'checkpoint-70001.ckpt')
...
#==============================================================================
    ...
    SPL=[]
    SR=[]
    pstate1,pstate2,pstate3,state,gstate,shortest,pre_action,colli_info = envs.reset()
...
#===============================================================================
    ...
    criteriony=torch.nn.BCEWithLogitsLoss()
    criterionx = torch.nn.MSELoss()
    criterionz=torch.nn.BCELoss()
    ...
    colli_info=torch.FloatTensor(np.float32(colli_info))
    ...
    rollout.colli_infos[0].copy_(colli_info)

    episode_rewards = torch.zeros(num_envs, 1)
    final_rewards   = torch.zeros(num_envs, 1)
    #========================================================================================
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    best_path = os.path.join(CHECK_DIR, 'best.ckpt') #checkpoint-1186703
    ...
    for i_update in range(num_frames):
        ...
        for step in range(num_steps):
            ...
            pim1,pim2,pim3,next_state,g_state, reward, done,gt_action,gt_state, shortest,pre_action, colli_info = envs.step(action.cpu().data.numpy())
            for i in range(num_envs):
                my_path[i][-1]+=1
                if reward[i]>5:
            ...
            rollout.insert(step,pstate1,pstate2,pstate3, state, gstate, action.unsqueeze(1).data, gt_action.unsqueeze(1).data,gt_state, reward, masks,pre_action,colli_info)
        with torch.set_grad_enabled(True):
            actor_critic.train()
            logit, gc, cc, z_mean,z_sigma, z,myaction  = actor_critic.evaluate_actions(
                ...
                Variable(rollout.pre_actions[:-1]).view(-1, 9),
            )
            rgt=rollout.gtactions.view(-1)
            action_loss1=criterion(logit,rgt)
            cgt=rollout.colli_infos[:-1].view(-1,7)
            action_loss2=criteriony(cc,cgt)
            bt_size=rgt.size()[0]
            #======================================================
            ...
            recon_loss1= criterionx(z_mean, p_mean)#torch.mean(1-cos_loss(z_mean,p_mean))#
            recon_loss2= criterionx(z_sigma, p_sigma)#torch.mean(1-cos_loss(z_sigma,p_sigma))#
            xnext=actor_critic.forward_once(Variable(rollout.gt_states).view(-1, *state_shape))
            recon_loss=criterionx(xnext, z)+criterionx(xnext, pz)#torch.mean(1-cos_loss(xnext,z))+torch.mean(1-cos_loss(xnext,pz))#
            ...
            c=torch.tensor(6, device='cuda:0')
            # msrgt=torch.zeros(bt_size,2)
            # msrgt=msrgt.cuda()
            # for idx in range(bt_size):
            #     if rgt[idx]==c:
            #         msrgt[idx][1]=b
            ...
            #         msrgt[idx][0]=b
            #================================================================
            # gc = torch.clamp(gc,min=1e-8,max=1-1e-8) 
            # action_loss3=0
            # for idx in range(bt_size):
            #     if rgt[idx]==c:
            #         action_loss3+=0.9*torch.log(gc[idx][1])
            #     else:
            #         action_loss3+=0.1*torch.log(gc[idx][0])
            # action_loss3=torch.neg(action_loss3/bt_size)
            #================================================================
            msrgt=torch.zeros(bt_size,2)
            for idx in range(bt_size):
                if rgt[idx]==c:
                    ...
                    msrgt[idx][0]=1.
            msrgt=msrgt.cuda()
            action_loss3=criterionz(gc,msrgt)
            #====================================================
            loss=action_loss1+0.4*action_loss2+0.4*action_loss3+0.1*recon_loss+0.0001*kl_loss
            loss.backward()
            global_t+=1
        ...
        if global_t%5==0:
            print("i_ipdates:",i_update,'| loss:', loss.item(), '| action_loss1:', action_loss1.item(),'| action_loss2:', action_loss2.item(),'| action_loss3:', action_loss3.item()) 
            ...
            writer.add_scalar('loss/gt_action_loss1',action_loss1.item(),global_t)
            writer.add_scalar('loss/gt_action_loss2',action_loss2.item(),global_t)
            writer.add_scalar('loss/kl-loss',kl_loss.item(),global_t)
            writer.add_scalar('loss/recon_loss',recon_loss.item(),global_t)
            #writer.add_scalar('loss/policy_action_loss',action_loss.item(),global_t)
        ...
        #============================================================
        if i_update>0 and i_update%200==0:
            spl=[]
            sr=[]
            for i in range(num_envs):
